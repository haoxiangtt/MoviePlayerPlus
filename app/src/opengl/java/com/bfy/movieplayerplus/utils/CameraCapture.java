...
public class CameraCapture {
    ...
    public void openBackCamera() {
        LogUtils.i(TAG, "Back Camera open....");
        int numberOfCameras = Camera.getNumberOfCameras();
        Camera.CameraInfo cameraInfo = new Camera.CameraInfo();
        for (int i = 0; i < numberOfCameras; i++) {
            Camera.getCameraInfo(i, cameraInfo);
            if (cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_BACK) {
                LogUtils.i(TAG,"Back Camera open over....");
                mCamera = Camera.open(i);
                return;
            }
        }
        LogUtils.i(TAG, "Back Camera open 异常!!!");
        doStopCamera();
    }
    ...
    public void openFrontCamera() {
        LogUtils.i(TAG, "Front Camera open....");
        int numberOfCameras = Camera.getNumberOfCameras();
        Camera.CameraInfo cameraInfo = new Camera.CameraInfo();
        for (int i = 0; i < numberOfCameras; i++) {
            Camera.getCameraInfo(i, cameraInfo);
            if (cameraInfo.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
                LogUtils.i(TAG, "Front Camera open over....");
                mCamera = Camera.open(i);
                return;
            }
        }
        LogUtils.i(TAG, "Front Camera open 异常!!!");
        doStopCamera();
    }
    ...
    private void initCamera() {
        if (mCamera != null) {
            ...
            LogUtils.i(TAG, "最终设置:PreviewSize--With = " + mParams.getPreviewSize().width
                    + "Height = " + mParams.getPreviewSize().height);
            LogUtils.i(TAG, "最终设置:PictureSize--With = " + mParams.getPictureSize().width
