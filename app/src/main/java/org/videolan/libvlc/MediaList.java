...
public class MediaList implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4320477910415398440L;
     ...
     * add by haoxiangtt
     */
    public void add(MediaList list, boolean noVideo){
    	add(list, noVideo, false);
    }
    
    /**
     * add by haoxiangtt
     */
    public void add(MediaList list, boolean noVideo, boolean noHardwareAcceleration){
    	for(int i = 0 ; i < list.size() ; i++){
    		Media media = list.getMedia(i);
    		mInternalList.add(new MediaHolder(media, noVideo, noHardwareAcceleration));
    	    signal_list_event(EventHandler.CustomMediaListItemAdded, mInternalList.size() - 1, media.getLocation());
    	}
    }
